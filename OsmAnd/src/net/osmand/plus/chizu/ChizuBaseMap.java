...
public class ChizuBaseMap {
	...
	/**
	 * Renders the block and writes it to {@link Params#outPath}, overwriting whatever was there.
	 *
	 * <p>The PNG appears at its final name only once it is whole: it is written beside itself and
	 * renamed into place. 自由作業盤 caches what it gets and blanks every walk in the area if the
	 * file is wrong, so a half-written picture under the right name is worse than no reply at all.
	 */
	@NonNull
	public static Result render(@NonNull OsmandApplication app, @NonNull Params params)
			throws TrackError {
		validate(params);
		File out = target(app, params.outPath);
		...
		result.outPath = out.getAbsolutePath();
	...
	private static File target(@NonNull OsmandApplication app, @Nullable String outPath)
		...
		if (outPath == null || outPath.trim().isEmpty()) {
			throw new TrackError("no out_path");
		}
